...
class OpenBCIBoard(object):
  ...
  def __init__(self, port=None, baud=115200, filter_data=True,
    ...
    time.sleep(1)
    # there's a built in delay in print_incoming_text() function
  ...
  def start_streaming(self,callback,duration = 60, lapse=-1):
    """
    ...
    while self.streaming and time.time() < end_time:
  ...
  """
  def _read_serial_binary(self, max_bytes_to_skip=300):
    def read(n):
      b = self.ser.read(n)
      if not b:
      ...
      #---------End Byte---------
        ...
        if (val == END_BYTE):
          ...
          if self.packets_dropped > max_packets_to_skip:
            print("too many packets skipped, reconnecting...")
            self.reconnect()
  ...
  def disconnect(self):
    ...
    if (self.ser.isOpen()):
      self.ser.close()
    if not self.check_connection_timer==None:
  ...
  #   self.check_connection_timer = threading.Timer(interval, self.check_connection)
